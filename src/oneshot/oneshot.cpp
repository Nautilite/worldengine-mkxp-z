...
static int linux_dialog(void *rawData) {
  ...
  GtkWidget *dialog = gtk_message_dialog_new(NULL, GTK_DIALOG_MODAL, gtktype,
                                             gtkbuttons, "%s", data->body);
...
Oneshot::Oneshot(RGSSThreadData &threadData) : threadData(threadData) {
  ...
  p->savePath = threadData.config.customDataPath.substr(
      0, threadData.config.customDataPath.size() - 1);
  ...
  // Get language code
  WCHAR wlang[9];
  GetLocaleInfoW(LOCALE_USER_DEFAULT, LOCALE_SISO639LANGNAME, wlang,
                 sizeof(wlang) / sizeof(WCHAR));
  p->lang = w32_fromWide(wlang) + "_";
  GetLocaleInfoW(LOCALE_USER_DEFAULT, LOCALE_SISO3166CTRYNAME, wlang,
                 sizeof(wlang) / sizeof(WCHAR));
...
// Get documents path
  ...
  Debug() << "Desktop env  :" << desktop;
#endif
}

Oneshot::~Oneshot() { delete p; }
...
void Oneshot::update() {
  if (p->winPosChanged) {
    ...
    for (int i = 0, max = display_count; i < max; ++i) {
      ...
      // If it's entirely within the bounds of the screen, we don't need to
      // check out any other monitors
      if (intersect.x == 0 && intersect.y == 0 && intersect.w == 640 &&
          intersect.h == 480)
        return;

      for (int y = intersect.y; y < intersect.y + intersect.h; ++y) {
        int start = y * 640 + intersect.x;
        std::fill(obscuredFrame.begin() + start,
                  obscuredFrame.begin() + (start + intersect.w), false);
      }
    }

    // Update the obscured map, and return prematurely if we don't have any
    // changes to make to the texture
...
const std::string &Oneshot::os() const { return p->os; }

const std::string &Oneshot::lang() const { return p->lang; }

const std::string &Oneshot::userName() const { return p->userName; }

const std::string &Oneshot::savePath() const { return p->savePath; }

const std::string &Oneshot::docsPath() const { return p->docsPath; }

const std::string &Oneshot::gamePath() const { return p->gamePath; }

const std::string &Oneshot::journal() const { return p->journal; }

const std::vector<uint8_t> &Oneshot::obscuredMap() const {
  return p->obscuredMap;
}

bool Oneshot::obscuredCleared() const { return p->obscuredCleared; }

bool Oneshot::exiting() const { return p->exiting; }

bool Oneshot::allowExit() const { return p->allowExit; }
...
#ifdef _WIN32
    title =
        "\u200b"; // Zero width space instead of filename in messagebox title
#else
    title = "";
#endif
...
#else
  // Buttons data
  static const SDL_MessageBoxButtonData buttonOk = {
      SDL_MESSAGEBOX_BUTTON_RETURNKEY_DEFAULT, 1, "OK"};
  static const SDL_MessageBoxButtonData buttonsOk[] = {buttonOk};
  SDL_MessageBoxButtonData buttonYes = {SDL_MESSAGEBOX_BUTTON_RETURNKEY_DEFAULT,
                                        1, p->txtYes.c_str()};
  SDL_MessageBoxButtonData buttonNo = {SDL_MESSAGEBOX_BUTTON_ESCAPEKEY_DEFAULT,
                                       0, p->txtNo.c_str()};
...
#ifdef OS_OSX
  int *btn = &button;
  // Message boxes and UI changes must be performed from the main thread on
  // macOS Mojave and above. This block ensures the message box will show from
  // the main thread.
