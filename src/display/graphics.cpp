...
struct Movie {
  ...
  Movie(bool skippable_)
      : decoder(0), video(0), skippable(skippable_), videoBitmap(0)
#ifndef MKXPZ_NO_OPENAL
        ,
        audioThread(0), audio(0)
#endif
  {
  }
  bool preparePlayback() {
    ...
    io->close = closeMovie;
    io->userdata = &srcOps;
    decoder = THEORAPLAY_startDecode(io, DEF_MAX_VIDEO_FRAMES,
    ...
    if (hasAudio) {
      while ((audio = THEORAPLAY_getAudio(decoder)) == NULL) {
        if ((THEORAPLAY_availableVideo(decoder) >= DEF_MAX_VIDEO_FRAMES)) {
          break; // we'll never progress, there's no audio yet but we've
                 // prebuffered as much as we plan to.
        }
        SDL_Delay(VIDEO_DELAY);
      }
    }
  ...
  void bufferMovieAudio(THEORAPLAY_Decoder *decoder, const Uint32 now) {
    ...
    while ((audio = THEORAPLAY_getAudio(decoder)) != NULL) {
      ...
      if (audio->playms >=
          now + AUDIO_BUFFER_LEN_MS) { // don't let this get too far ahead.
        break;
      }
    }
  }
    ...
    while (true) {
      while (procBufs--) {
        ...
        while (audioQueueHead && (remainingSamples > 0)) {
          ...
          sourceSamples = audioPacketAndOffset->audio->samples +
                          (audioPacketAndOffset->offset * channels);
          samplesToProcess = (audioPacketAndOffset->audio->frames -
                              audioPacketAndOffset->offset) *
                             channels;
          ...
          if ((audioPacketAndOffset->offset) >=
              audioPacketAndOffset->audio->frames) {
        ...
        alBufferData(
            alBuffers[procBufs],
            channels == 1 ? AL_FORMAT_MONO16 : AL_FORMAT_STEREO16, audioBuffer,
            (MOVIE_AUDIO_BUFFER_SIZE - remainingSamples) * sizeof(ALshort),
            sampleRate);
  ...
  bool startAudio(float volume) {
    ...
    audioThread =
        createSDLThread<Movie, &Movie::streamMovieAudio>(this, "movieaudio");
  ...
  void play(float volume) {
    ...
    while (THEORAPLAY_isDecoding(decoder)) {
      ...
      if (skippable) {
        ...
        if (shState->input().isTriggered(Input::Action) ||
            shState->input().isTriggered(Input::Cancel))
          break;
      }
      ...
      if (video && (video->playms <= now)) {
        ...
        videoBitmap->replaceRaw(video->pixels,
...
struct MovieOpenHandler : FileSystem::OpenHandler {
  ...
  MovieOpenHandler(SDL_IOStream *srcOps) : srcOps(srcOps) {}
...
PingPong::PingPong(int screenW, int screenH)
...
TEXFBO &PingPong::backBuffer() { return rt[srcInd]; }

TEXFBO &PingPong::frontBuffer() { return rt[dstInd]; }

/* Better not call this during render cycles */
void PingPong::resize(int width, int height) {
  screenW = width;
  screenH = height;
...
void PingPong::startRender() { bind(); }

void PingPong::swapRender() {
...
void ScreenScene::composite() {
  ...
  if (!transparent) // clear background to black unless transparent
    gl.ClearColor(0, 0, 0, 1);
  FBO::clear();
  if (shState->config().transparentDefault)
...
void ScreenScene::requestViewportRender(const Vec4 &c, const Vec4 &f,
                                        const Vec4 &t, const bool s,
                                        const Vec4 rx, const Vec4 ry,
                                        const float cubic) {
...
void ScreenScene::setBrightness(float norm) {
  brightnessQuad.setColor(Vec4(0, 0, 0, 1.0f - norm));

  brightEffect = norm < 1.0f;
}

void ScreenScene::updateReso(int width, int height) {
...
void ScreenScene::setResolution(int width, int height) {
  pp.resize(width, height);
  updateReso(width, height);
}

PingPong &ScreenScene::getPP() { return pp; }
...
struct GraphicsPrivate {
  ...
  GraphicsPrivate(RGSSThreadData *rtData)
      : scResLores(DEF_SCREEN_W, DEF_SCREEN_H),
        scRes(rtData->config.enableHires
                  ? (int)lround(rtData->config.framebufferScalingFactor *
                                DEF_SCREEN_W)
                  : DEF_SCREEN_W,
              rtData->config.enableHires
                  ? (int)lround(rtData->config.framebufferScalingFactor *
                                DEF_SCREEN_H)
                  : DEF_SCREEN_H),
        scSize(scRes),
        ...
        frozen(false), last_update(0), last_avg_update(0),
        backingScaleFactor(1), integerScaleFactor(0, 0),
    ...
    TEX::setRepeat(false);
    TEX::setSmooth(false);
    gl.TexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE8, 640, 480, 0, GL_LUMINANCE,
                  GL_UNSIGNED_BYTE, 0);
  }
  ...
  void recalculateScreenSize(bool fixedAspectRatio) {
    ...
    if (!fixedAspectRatio) {
      if (!integerScaleActive ||
          (integerScaleActive && integerLastMileScaling)) {
        scOffset = Vec2i(0, 0);
        return;
      }
    }
    ...
    if (integerScaleActive && !integerLastMileScaling) {
      ...
      scSize =
          Vec2i(scRes.x * integerScaleFactor.x, scRes.y * integerScaleFactor.y);
      return;
    }
  ...
  bool integerScaleStepApplicable() const {
    ...
    if (integerScaleFactor.x < 1 ||
        integerScaleFactor.y < 1) // XXX should be < 2, this is for testing only
      return false;
  ...
  void compositeToBufferScaled(TEXFBO &buffer, int destWidth, int destHeight) {
    ...
    int scaleIsSpecial = GLMeta::blitScaleIsSpecial(
        buffer, false, IntRect(0, 0, destWidth, destHeight),
        screen.getPP().frontBuffer(), IntRect(0, 0, scRes.x, scRes.y));
    ...
    GLMeta::blitRectangle(IntRect(0, 0, scRes.x, scRes.y),
                          IntRect(0, 0, destWidth, destHeight));
    GLMeta::blitEnd();
  }
  ...
  void metaBlitBufferFlippedScaled(const Vec2i &sourceSize, int scaleIsSpecial,
                                   bool forceNearestNeighbor = false) {
    GLMeta::blitRectangle(
        IntRect(0, 0, sourceSize.x, sourceSize.y),
        IntRect(scOffset.x, scSize.y + scOffset.y, scSize.x, -scSize.y),
        !forceNearestNeighbor &&
            GLMeta::smoothScalingMethod(scaleIsSpecial) == Bilinear);
  }
  ...
  void redrawScreen() {
    if (shState->oneshot().obscuredDirty) {
      TEX::bind(obscuredTex);
      TEX::uploadSubImage(0, 0, 640, 480,
                          shState->oneshot().obscuredMap().data(),
                          GL_LUMINANCE);
    ...
    if (integerScaleStepApplicable() && !integerLastMileScaling) {
      int scaleIsSpecial = GLMeta::blitScaleIsSpecial(
          integerScaleBuffer, false, IntRect(0, 0, scSize.x, scSize.y),
          screen.getPP().frontBuffer(), IntRect(0, 0, scRes.x, scRes.y));
    ...
    int scaleIsSpecial = GLMeta::blitScaleIsSpecial(
        integerScaleBuffer, false, IntRect(0, 0, scSize.x, scSize.y),
        integerScaleActive ? integerScaleBuffer : screen.getPP().frontBuffer(),
        IntRect(0, 0, sourceSize.x, sourceSize.y));
...
void Graphics::update(bool checkForShutdown) {
  ...
  // update Input.repeat timing, rounding the framerate to the nearest 2
  {
    ...
    double afr = std::abs(
        averageFrameRate()); // abs shouldn't be necessary but that's ok
    afr += mult / 2;
...
void Graphics::transition(int duration, const char *filename, int vague) {
  ...
  for (int i = 0; i < duration; ++i) {
    ...
    int scaleIsSpecial = GLMeta::blitScaleIsSpecial(
        p->integerScaleBuffer, false, IntRect(0, 0, p->scSize.x, p->scSize.y),
        transBuffer, IntRect(0, 0, p->scRes.x, p->scRes.y));
...
void Graphics::fadeout(int duration) {
  ...
  for (int i = duration - 1; i > -1; --i) {
    ...
    if (p->frozen) {
      int scaleIsSpecial = GLMeta::blitScaleIsSpecial(
          p->integerScaleBuffer, false, IntRect(0, 0, p->scSize.x, p->scSize.y),
          p->frozenScene, IntRect(0, 0, p->scRes.x, p->scRes.y));
...
int Graphics::displayWidth() const {
  const SDL_DisplayMode *dm =
      SDL_GetCurrentDisplayMode(SDL_GetDisplayForWindow(shState->sdlWindow()));
...
void Graphics::playMovie(const char *filename, int volume_, bool skippable) {
  ...
  if (movie->preparePlayback()) {
    ...
    // Currently this stretches to fit the screen. VX Ace behavior is to center
    // it and let the edges run off
    movieSprite.setBitmap(movie->videoBitmap);
    double ratio = std::min((double)width() / movie->video->width,
                            (double)height() / movie->video->height);
...
void Graphics::setBorderless(bool value) {
  p->threadData->ethread->requestBorderlessMode(value);
}
...
void Graphics::repaintWait(const AtomicFlag &exitCond, bool checkReset) {
  ...
  int scaleIsSpecial = GLMeta::blitScaleIsSpecial(
      p->integerScaleBuffer, false, IntRect(0, 0, p->scSize.x, p->scSize.y),
      lastFrame, IntRect(0, 0, p->scRes.x, p->scRes.y));
