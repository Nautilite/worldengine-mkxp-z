...
#define AL_FUN(name, type)                                                     \
  alc.name = (type)alcGetProcAddress(alcDev, "alc" #name "SOFT");
  AL_DEVICE_PAUSE_FUN;
#undef AL_FUN
}
...
EventThread::EventThread() : ctrl(0), fullscreen(false), showCursor(false) {
...
void EventThread::process(RGSSThreadData &rtData) {
  ...
  SDL_AddGamepadMappingsFromFile(
      mkxp_fs::getPathForAsset("gamecontrollerdb", "txt").c_str());
#else
  SDL_AddGamepadMappingsFromIO(
      SDL_IOFromConstMem(___assets_gamecontrollerdb_txt,
  ...
  while (true) {
    ...
    case SDL_EVENT_KEY_DOWN:
      ...
      if (event.key.scancode == SDL_SCANCODE_F1 &&
          rtData.config.enableSettings) {
        // Do not open settings menu until initializing shared state.
        // Opening before initializing shared state will crash (segmentation
        // fault).
        if (!shState) {
          break;
        }
      ...
      if (acceptingTextInput && event.key.key == SDLK_BACKSPACE) {
        ...
        while (textInputBuffer.length() != 0 &&
               (textInputBuffer.back() & 0xc0) == 0x80) {
    ...
    default:
      ...
      case REQUEST_WINCENTER: {
        ...
        if (!rc)
          SDL_SetWindowPosition(win, rect.x + (dm.w / 2) - (winW / 2),
      ...
      case REQUEST_MESSAGEBOX: {
        ...
        std::string message =
            copyWithNewlines((const char *)event.user.data1, 70);
      ...
      case REQUEST_NEW_WINDOW: {
        const CreateWindowArgs *args =
            static_cast<const CreateWindowArgs *>(event.user.data1);
...
bool EventThread::eventFilter(void *data, SDL_Event *event) {
  ...
  default:
    // sdl3 removed SDL_WINDOWEVENT, so we just check if the type is in the
    // expected range
    if (event->type >= SDL_EVENT_WINDOW_FIRST &&
        event->type <= SDL_EVENT_WINDOW_LAST) {
      {
        ...
        if (win_id !=
            event->window.windowID) // filter out events from other windows
          return false;
      }

      if (event->type == SDL_EVENT_WINDOW_MOVED) {
        if (shState != NULL && shState->rgssVersion > 0) {
          shState->oneshot().setWindowPos(event->window.data1,
                                          event->window.data2);
...
void EventThread::setFullscreen(SDL_Window *win, bool mode) {
  SDL_SetWindowFullscreen(win, mode ? SDL_WINDOW_FULLSCREEN : 0);
  fullscreen = mode;
}
...
void EventThread::requestBorderlessMode(bool mode) {
  if (mode == borderless)
    return;
  SDL_Event event;
  setBorderless(event.window, mode);
}
...
SDL_Window *EventThread::requestNewWindow(const CreateWindowArgs *args) {
  ...
  while (!new_window)
    ...
    // ideally the delay should be long enough that the window is created in one
    // loop, but short enough that we aren't waiting for a long time after the
    // window is created
    SDL_DelayNS(10000);
...
SDL_Gamepad *EventThread::controller() const { return ctrl; }
